...
public:
    ...
    virtual void GetUnadjustedPositionForTime(
        const SimTime& snapshotTime, ObjectMobilityPosition& position) override
    {
...
public:
    ...
    ObjectMobilityPosition GetCurrentLocation(const SimTime& currentTime) const {
...
private:
    void EnableMovingObjects(
        ...
        gisSubsystem.RemoveMovingObject(theNodeId);
...
SimTime Agent::CalculateWakeupTime()
{
                ...
                if (currentBehaviorPtr != nullptr) {
                    ...
                    if (!currentRouteList.IsEmpty()) {
                        if (IsFreeWalkPosition(desiredNextPositionId.type)) {
                            const SimTime preparetionTime = 30*SECOND;
    ...
    const bool byCommunication)
{
    ...
    const SimTime currentTime = simulatorPtr->CurrentTime();
    ...
    const map<DynamicApplicationIdType, DynamicApplicationDefinition>& applicationSpecifications)
{
    ...
    const AgentResource resource(this);
    const NodeId theNodeId = agentId;
    const SimTime currentTime = simulatorPtr->CurrentTime();
    ...
    for(IterType iter = applicationSpecifications.begin();
        ...
        if (!instanceName.empty()) {
            instanceId = instanceName + "-" + ConvertToString(theNodeId);
        } else {
            instanceId = "dynamicapp" + ConvertTimeToStringSecs(currentTime) + "s-" + ConvertToString(theNodeId);
        }

        ostringstream scopeStream;

        scopeStream << "[" << theNodeId << ";" << instanceId << "]";
        ...
        for(size_t i = 0; i < parameters.size(); i++) {
            ...
            if (parameter.applicationParameterName.find("-destination") != string::npos) {
                const NodeId destinationNodeId = App_ConvertStringToNodeIdOrAnyNodeId(parameter.value);
...
void Agent::RecalculateBehaviorIfNecessary(bool& assignedNewBehavior)
{
    ...
    const SimTime currentTime = resource.CurrentTime();
...
void Agent::IncrementCurrentBehaviorTime()
{
    ...
    if (!recalculateRoute && currentBehaviorPtr->HasFinished()) {
        ...
        if (currentRouteList.routePtrs.empty() && (*this).HasCurrentTask()) {
            const SimTime currentTime = resource.CurrentTime();
            const SimTime waitTime = (*this).CurrentTask().GetWaitTime(resource);
            const SimTime waitEndTime = currentTime + waitTime;
...
void Agent::RecalculateRoute(
    const SimTime& recalculateStartTime,
...
SimTime AgentResource::CurrentTime() const
{
    (*this).CheckAgentAvailability();
...
SimTime AgentResource::CurrentBehaviorSpentTime() const
{
    (*this).CheckAgentAvailability();
...
ObjectMobilityPosition AgentResource::MobilityPosition() const
{
    ...
    const SimTime currentTime = (*this).CurrentTime();
...
ObjectMobilityPosition AgentResource::MobilityPositionForTime(const SimTime& time) const
{
    (*this).CheckAgentAvailability();
...
bool AgentResource::ExceededWaitEntranceTime() const
{
    ...
    const SimTime waitTime = std::max(
...
SimTime AgentResource::LastDelay() const
{
    (*this).CheckAgentAvailability();
...
SimTime AgentResource::NextDelay() const
{
    (*this).CheckAgentAvailability();
...
SimTime AgentResource::TripDelay() const
{
    (*this).CheckAgentAvailability();
...
SimTime AgentResource::ArrivalDelay() const
{
    (*this).CheckAgentAvailability();
...
bool AgentResource::ExceededRouteRecalculationTime() const
{
    ...
    const SimTime recalculationTime = (*this).RouteRecalculationTime();
...
bool AgentResource::IsPathQueryTriggerAvailable(
    ...
    const SimTime& vehicleDelay) const
{
    (*this).CheckAgentAvailability();
    ...
    const SimTime minPathQueryDuration = (*this).MinPathQueryInterval();
...
void AgentResource::RecalculateRoute(const SimTime& recalculateStartTime)
{
    (*this).CheckAgentAvailability();
...
SimTime AgentValueFormula::CalculateTime(
    const AgentResource& resource,
    HighQualityRandomNumberGenerator& randGen,
    const bool calculateMaxValue) const
{
    ...
    return static_cast<SimTime>((*this).CalculateDouble(resource, randGen, calculateMaxValue) * SECOND);
}
...
SimTime AgentTask::GetStartTime(const AgentResource& resource) const
{
    if (startTime.IsNull()) {
        return ZERO_TIME;
    }
...
SimTime AgentTask::GetEndTime(const AgentResource& resource) const
{
    if (endTime.IsNull()) {
        return INFINITE_TIME;
    }
...
SimTime AgentTask::GetWaitTime(const AgentResource& resource) const
{
    if (waitTime.IsNull()) {
        return ZERO_TIME;
    }
    ...
    priority_queue_stable<AgentStatusChangeEvent>& timeLineStatusChangeEvents) const
{
    ...
    for(size_t i = 0; i < statusChangePtrs.size(); i++) {
        ...
        const SimTime changeTime = statusChangeTask.GetStartTime(resource);
    ...
    initialLocationFileCachePtr(new InorderFileCache())
{
    ...
    for(IterType iter = agentIds.begin(); iter != agentIds.end(); iter++) {
        ...
        if (AStringStartsWith(profileName, "taxi")) {
            ...
            const SimTime wakeupTime = agentPtr->CalculateWakeupTime();
...
void MultiAgentSimulator::RunSimulationUntil(const SimTime& snapshotTime)
{
...
bool MultiAgentSimulator::IsEqualToAgentId(const NodeId& theNodeId) const
{
    return (entireAgentIds.find(theNodeId) != entireAgentIds.end());
}

SimTime MultiAgentSimulator::GetWakeupTime(const NodeId& theNodeId) const
{
    assert((*this).IsEqualToAgentId(theNodeId));

    typedef map<NodeId, SimTime>::const_iterator IterType;

    IterType iter = wakeupTimes.find(theNodeId);
...
void MultiAgentSimulator::OutputTraceEvent(
    const NodeId& gisObjectOrAgentId,
...
void MultiAgentSimulator::CreateApplicationForNode(
    ...
    const set<NodeId>& targetNodeIds)
{
...
{
    const NodeId& sourceNodeId = dynamicApplicationData.sourceNodeId;
    const InterfaceOrInstanceId& instanceId = dynamicApplicationData.instanceId;
    const vector<string>& parameterLines = dynamicApplicationData.parameterLines;
    const set<NodeId>& targetNodeIds = dynamicApplicationData.targetNodeIds;
    ...
    if (targetNodeIds.find(ANY_NODEID) != targetNodeIds.end()) {
        ...
        for(IterType iter = targetNodeIds.begin(); iter != targetNodeIds.end(); iter++) {
            ...
            if (nodes.find(theNodeId) == nodes.end()) {
                cerr << "Error: Network Node Id: " << theNodeId << " Not Found." << endl;
                exit(1);
            }

            const shared_ptr<NetworkNode> networkNodePtr = nodes[theNodeId];

            if (networkNodePtr == nullptr) {
                cerr << "Error: Network Node Id: " << theNodeId << " is not NetworkNode." << endl;
                exit(1);
            }
